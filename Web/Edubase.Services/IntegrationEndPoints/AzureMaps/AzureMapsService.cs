...
public class AzureMapsService : IAzureMapsService
{
        ...
        using (var response = await RetryPolicy.ExecuteAsync(async () =>
               {
                   ...
                   {
                       return await _azureMapsClient.SendAsync(
                           request,
            ...
            using (JsonReader reader = new JsonTextReader(sr))
            {
                ...
                foreach (var municipality in municipalities)
                {
                    var child = subMunicipalities.FirstOrDefault(x =>
                        x.address.municipality == municipality.address.municipality
    ...
    private string GetAddressDescription(Result locationResult, string text)
    {
        ...
        if (locationResult.entityType != null && locationResult.entityType == "MunicipalitySubdivision")
        {
        ...
        if (locationResult.address.postalCode != null &&
            !output.Contains(locationResult.address.postalCode.Split(',')[0]))
        {
            if (text.IsUkPostCode())
            {
                if (locationResult.address.extendedPostalCode.ToLower().Remove(" ")
                    .Contains(text.ToLower().Remove(" ")))
                {
                    output += $", {text.ToUpper()}";
                }
                else
