...
namespace Edubase.Common.Cache
{
    public class CacheAccessor : ICacheAccessor, IDisposable
    {
        ...
        /// <param name="paramsCacheKey">
        ///     The main cache key comes from the caller type name and caller func. name. This is purely
        ///     the parameters, as a concatenated string
        /// </param>
        ...
        /// <returns></returns>
        public async Task<T> AutoAsync<T>(Func<Task<T>> asyncFactory, string paramsCacheKey, string callerTypeName,
            string relationshipKey = null, [CallerMemberName] string callerFuncName = null)
        {
        ...
        /// <param name="callerFuncName"></param>
        /// <returns></returns>
        public async Task<T> AutoAsync<T>(Func<T> factory, string paramsCacheKey, string callerTypeName,
            string relationshipKey = null, [CallerMemberName] string callerFuncName = null)
        {
            var key = $"{callerTypeName}.{callerFuncName}({paramsCacheKey})".ToLower();
            var retVal = await GetAsync<T>(key);
        ...
        /// <param name="callerFuncName"></param>
        /// <returns></returns>
        public T Auto<T>(Func<T> factory, string paramsCacheKey, string callerTypeName, string relationshipKey = null,
            [CallerMemberName] string callerFuncName = null)
        {
            var key = $"{callerTypeName}.{callerFuncName}({paramsCacheKey})".ToLower();
        ...
        public async Task ClearRelatedCacheKeysAsync(string relationshipKey)
        {
            ...
            if (data != null)
            {
                var cacheKeys = data.Split(new[] { ';' }, StringSplitOptions.RemoveEmptyEntries)
                    .Where(x => x.Clean() != null).ToArray();
        ...
        public long GetMemoryCacheApproximateSize()
        {
            ...
            var monitorField = statsValue.GetType()
                .GetField("_cacheMemoryMonitor", BindingFlags.NonPublic | BindingFlags.Instance);
            var monitorValue = monitorField.GetValue(statsValue);
            var sizeField = monitorValue.GetType()
                .GetField("_sizedRefMultiple", BindingFlags.NonPublic | BindingFlags.Instance);
            var sizeValue = sizeField.GetValue(monitorValue);
            var approxProp = sizeValue.GetType()
                .GetProperty("ApproximateSize", BindingFlags.NonPublic | BindingFlags.Instance);
        ...
        public static ICacheAccessor Create()
        {
            return new CacheAccessor(new JsonConverterCollection());
        }
        ...
        /// <summary>
        ///     Creates a memory cache that's used for frequently used immutable uncloned objects (v. fast, but immutable objects
        ///     only)
        /// </summary>
        /// <param name="config"></param>
        /// <returns></returns>
        ...
        public Task InitialiseIfNecessaryAsync()
        {
            ...
            {
                _connectingTask = Task.FromResult(null as object);
            }
        ...
        private void OnKeyValueUpdated(RedisChannel channel, RedisValue value)
        {
            try
            {
                if (value.HasValue)
                {
                    ...
                    if (message.SenderCacheName !=
                        Name) // don't process messages sent by the sender instance, as the job should have been synchronously
                    {
                        if (!_memoryCache.Contains(message
                                .TransactionCacheKey)) // checks whether the message has already been processed (i.e., this node did the op directly)
                        {
        ...
        public async Task<long> WaitForEvent(eCacheEvent cacheEvent, int maxWaitMilliseconds = 20000)
        {
            ...
            while (LastCachingEvent != cacheEvent)
            {
                ...
                {
                    throw new Exception(string.Format("Max wait time elapsed waiting for status '{0}', but was '{1}'",
                        cacheEvent, LastCachingEvent));
                }
        ...
        ///     This was introduced because during testing, you want to clear down the cache, but there may still be pending cache
        ///     messages
        ...
        /// </param>
        private void SetInMemoryCache(string key, byte[] data, DateTime? expirationUtc = null,
            bool recordAsSessionKey = true)
        {
            key = CleanKey(key);
        ...
        private void SetInMemoryCache(string key, byte[] value, TimeSpan? expiration = null,
            bool recordAsSessionKey = true)
        {
        ...
        private void Log(eCacheEvent cachingEvent, string key, [CallerMemberName] string callerName = null)
        {
            ...
            {
                AuditLog.Add(new CacheAuditLogItem
                {
        ...
        {
            return _config.Name + Environment.NewLine + string.Join(Environment.NewLine, AuditLog.Select(x =>
                string.Format("{0}: event:{1}, func:{2}, key:{3}",
        ...
        /// <summary>
        ///     Deletes the keys that have been cached during this instance. Useful for clearing the cache during a integration
        ///     test session.
        ///     Better than ClearAsync because we all share the dev cache and this will only remove items cached by the current
        ///     session
        /// </summary>
        /// <returns></returns>
        public void DeleteKeysInThisSession()
        {
            lock (_mutex2) // make thread-safe; this method doesn't support re-entrancy
            {
                if (ShouldRecordKeysInSession)
                {
                    ...
                    {
                        _memoryCache.Remove(key);
                        if (_cacheDatabase != null && Status == State.Connected)
                        {
                            _cacheDatabase.KeyDelete(key);
                        }
                    }
        ...
        ///     The method will return very quickly, because it'll set things to the cache asynchronously (the client code can be
        ///     agnostic about this fact).
        ...
        public async Task SetAsync<T>(string key, T value, TimeSpan? cacheExpiry = null)
        {
            ...
            try
            {
                ...
                var buffer = ToByteBuffer(value, _config.IsPayloadCompressionEnabled,
                    new Dictionary<string, object> { ["ExpirationUtc"] = expirationUtc });
                ...
                if (Status == State.Connected)
                {
                    ...
                    {
                        throw new Exception("The cache database is null, in-spite of it being connected");
                    }
        ...
        private async Task DistributeCacheItem(string key, byte[] data, DateTime? expirationUtc)
        {
            if (_config.IsDistributedCachingEnabled && _subscriber != null)
            {
                ...
                {
                    Key = key, Buffer = data, SenderCacheName = Name, ExpirationUtc = expirationUtc
                };
...
#pragma warning restore 4014
        ...
        private string CleanKey(string key)
        {
            ...
            {
                throw new Exception("The key was invalid");
            }
