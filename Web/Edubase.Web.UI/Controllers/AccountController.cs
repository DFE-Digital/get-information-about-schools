...
namespace Edubase.Web.UI.Controllers
{
    public class AccountController : Controller
    {
        ...
        private ApplicationSignInManager SignInHelper
        {
            get
            {
                if (_helper == null)
                {
                    _helper = new ApplicationSignInManager(UserManager, AuthenticationManager);
                }
                return _helper;
            }
        }
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            var result = await SignInHelper.PasswordSignInAsync(model.Username, model.Password, model.RememberMe, shouldLockout: false);
            switch (result)
            {
                ...
                case SignInStatus.Failure:
                    ModelState.AddModelError("", "We did not recognise the details that you entered. Please try again.");
                    ModelState.AddModelError("", "If you are having trouble logging in, please contact our helpdesk on telephone: 011 111 111.");
                    return View(model);
                default:
        ...
        public async Task<ActionResult> VerifyCode(string provider, string returnUrl)
        {
            ...
            if (!await SignInHelper.HasBeenVerifiedAsync())
            {
                return View("Error");
            }
        ...
        public async Task<ActionResult> VerifyCode(VerifyCodeViewModel model)
        {
            ...
            var result = await SignInHelper.TwoFactorSignInAsync(model.Provider, model.Code, isPersistent: false, rememberBrowser: model.RememberBrowser);
            switch (result)
            {
                case SignInStatus.Success:
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var emailConfirmed = !model.RequiresEmailConfirmation;
                var user = new ApplicationUser { UserName = model.Username, Email = model.Email, EmailConfirmed = emailConfirmed };
        ...
        public async Task<ActionResult> SendCode(SendCodeViewModel model)
        {
            ...
            if (!await SignInHelper.SendTwoFactorCodeAsync(model.SelectedProvider))
            {
                return View("Error");
            }
        ...
        {
            return RedirectToAction("AfterLogin", new { returnUrl });
        }

        public ActionResult AfterLogin(string returnUrl)
        {
            return _redirectAfterLoginHelper.GetResult(returnUrl);
        }
