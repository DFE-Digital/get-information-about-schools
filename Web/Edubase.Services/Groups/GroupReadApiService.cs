...
namespace Edubase.Services.Groups
{
    ...
    using Domain;
    using eStatus = Enums.eLookupGroupStatus;
    using Exceptions;
    using IntegrationEndPoints.AzureSearch;
    using IntegrationEndPoints.AzureSearch.Models;
    using Lookup;
    using Models;
    using Doc = Models.SearchGroupDocument;
    using MoreLinq;
    using Search;
    using Security;

    /// <summary>
    /// Implementation of IGroupReadService that will gradually be changed to call the API rather than custom backend
    /// </summary>
    public class GroupReadApiService : IGroupReadService
    {
        ...
        public GroupReadApiService(IApplicationDbContext dc, 
            IMapper mapper, 
