...
{
    public class NotificationsBannersAuditViewModel
    {
        public List<NotificationBanner> GroupedBanners { get; }

        public NotificationsBannersAuditViewModel(IEnumerable<NotificationBanner> items, string sortBy)
        {
            var sortOrder = "desc";
            ...
            if (sortOrder == "desc")
            {
                switch (sortBy)
                {
                    case nameof(NotificationBanner.Tracker):
