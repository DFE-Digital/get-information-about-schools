    ...
    OkCancel.prototype = {
        ...
        init: function () {
            ...
            var overlay = '<div id="'+opts.idPrefix+'modal-overlay" class="modal-overlay hidden"></div>' +
                ...
                '<div class="button-row"><a href="#" class="button js-allow-exit" id="' + opts.idPrefix + 'button-ok">' + (!opts.okLabel ? "OK" : opts.okLabel) + '</a>';

            if ($.isFunction(opts.cancel)) {
                overlay += '<a href="#" class="button button-secondary js-allow-exit" id="' + opts.idPrefix + 'button-cancel">Cancel</a>';
            }
            overlay += '</div></div>';
