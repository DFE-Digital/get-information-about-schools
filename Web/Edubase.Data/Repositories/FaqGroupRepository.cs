...
namespace Edubase.Data.Repositories
{
    public class FaqGroupRepository : TableStorageBase<FaqGroup>
    {
        ...
        public async Task CreateAsync(FaqGroup entity) => await Table.ExecuteAsync(TableOperation.Insert(entity));
        ...
        public async Task CreateAsync(IEnumerable<FaqGroup> entities) => await CreateAsync(entities.ToArray());

        public async Task<Page<FaqGroup>> GetAllAsync(int take, TableContinuationToken skip = null)
        {
            var query = Table.CreateQuery<FaqGroup>().AsQueryable();
            query = query.Take(take);
            var results = await Table.ExecuteQuerySegmentedAsync(query.AsTableQuery(), skip);
            return new Page<FaqGroup>(results, results.ContinuationToken);
        }

        public async Task<FaqGroup> GetAsync(string id)
        {
            var q = Table.CreateQuery<FaqGroup>().Where(x => x.PartitionKey == string.Empty && x.RowKey == id).AsTableQuery();
        ...
        public async Task UpdateAsync(FaqGroup item) => await Table.ExecuteAsync(TableOperation.Replace(item));
